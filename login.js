...
function isValidUser(userOb){
  if(!userOb){
    return false;
  }
  if(!userOb.username || !users.hasOwnProperty(userOb.username)){
    return false;
  }
  return true;
}

function getUser(requestingUser, userNum){
    ...
    if(users.hasOwnProperty(userNum)){
      ...
      if(requestingUser.username == userNum){
